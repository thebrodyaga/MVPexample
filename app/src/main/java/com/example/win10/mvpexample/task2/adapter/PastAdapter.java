...
import com.example.win10.mvpexample.task2.PastFragment.OnListFragmentInteractionListener;
...
public class PastAdapter extends RecyclerView.Adapter<PastAdapter.ViewHolder> {
    ...
    private final List<Past> mValues;
    private final OnListFragmentInteractionListener mListener;

    public PastAdapter(List<Past> items, OnListFragmentInteractionListener listener) {
        mValues = items;
        mListener = listener;
    }
    ...
    public void addAll(List<Past> list) {
        mValues.addAll(list);
        notifyDataSetChanged();
    }
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        ...
        if (holder.timer == null) {
            final long millis = mValues.get(position).getTime();
            holder.timer = new CountDownTimer(millis, 1000) { // adjust the milli seconds here
                public void onTick(long millisUntilFinished) {
                    long seconds = (millis + (millis - millisUntilFinished)) / 1000;
